...
export function investmentCommitteeHtml(deal: Deal) {
  ...
  const sections: [string, string][] = [
    ...
    ["10. Customer Concentration", `${d?.concentrationFlag || "UNKNOWN"}. ${d?.customers.map((c) => `${c.name} ${pct(c.share)}`).join("; ") || "NOT PROVIDED"}`],
    ...
    ["26. Customer Questions", (d?.customers[0]?.interviewQuestions || []).join(" ")],
